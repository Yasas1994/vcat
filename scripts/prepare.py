...
ictv["IDS"] = ictv.apply(
    lambda x: extract_from_hyperlink(x["Accessions Link"])["url"]
    .split("/")[-1]
    .split(","),
    axis=1,
)
ictv["Range"] = ictv.apply(
    lambda x: extract_regions(x["Virus GENBANK accession"]), axis=1
)
...
id2range = {}
for row in ictv[
    ["Taxid", "Virus name(s)", "IDS", "Range"]
].iterrows():  # ID = genome ID
    for i in row[1]["IDS"]:
        i = i.split("(")[0]
            ...
            if record.version.split(".")[0] in all_nrids:
                ...
                for feature in record.features:
                    if feature.key == "CDS":
                        ...
                        start, end = list(
                            map(int, re.findall(r"(\d+)\D+(\d+)", feature.location)[0])
                        )
                        ...
                        if (
                            min(feature_range[0], start) == feature_range[0]
                            and max(feature_range[1], end) == feature_range[1]
                        ):
                    ...
                    for feature in record.features:
                        if feature.key == "CDS":
                            ...
                            start, end = list(
                                map(
                                    int,
                                    re.findall(r"(\d+)\D+(\d+)", feature.location)[0],
                                )
                            )
                            # get start and end positions of the feature
                            # print(feature.location, start, end)

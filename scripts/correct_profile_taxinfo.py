...
# input m8 file
PROF = argv[2] #"/media/ssd/ICTV-TaxonomyChallenge/vcat/databases/VMR_latest/cuttoffs/prof/genomes_fna_prof.m8"
...
clustlca = pl.read_csv(f'{DATABASE_DIR}/VMR_latest/mmseqs_pprofiles/mmseqs_pprofiles_lca.tsv',
                       separator="\t")

clustlca = clustlca.with_columns(
    pl.col('taxid').map_elements(lambda x: taxopy.Taxon(x, taxdb).name, return_dtype=str).alias("taxname"),
    pl.col('taxid').map_elements(lambda x: str(taxopy.Taxon(x, taxdb)), return_dtype=str).alias("taxlineage"),
    pl.col('taxid').map_elements(lambda x: taxopy.Taxon(x, taxdb).rank, return_dtype=str).alias("taxrank"),
    )


ictv_prof = pl.read_csv(PROF,
                    has_header = False,
                    separator="\t",
                    new_columns=HEADER.split(","))

ictv_prof = ictv_prof.drop(["taxid", "taxname", "taxlineage"]).join(clustlca.drop(['root_p1', 'taxrank'])
                                                        , on="target", how="inner")
